...
# Between treatments
model <- glmer.nb(Seed ~ Treatment +(1|Plant.ID),
...
model <- glmer.nb(Seed ~ Morph +(1|Plant.ID),
               family = poisson, data = df)
summary(model)

#Between outcrossing and selfing treatments:

model <- glmer(Seed ~ Cross +(1|Plant.ID),
               family = poisson, data = df)
summary(model)

#Between inter-morph and intra-morph treatments:

model <- glmer(Seed ~ Type +(1|Plant.ID),
               family = poisson, data = df)
summary(model)
...
df <- df %>% 
  group_by(Treatment) %>%
  summarise(mn = mean(Seed, na.rm = T),
            sd = sd(Seed, na.rm = T),
            se = sd/sqrt(n()))
